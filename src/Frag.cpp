...
Frag::~Frag()
{
    for (vector<b2Body*>::iterator iter = mPartialofFrags.begin(); iter != mPartialofFrags.end(); iter++) {

            mWorld->DestroyBody(*iter);
    }
...
Frag::render()
{
    ...
    for (vector<b2Body*>::iterator iter = mPartialofFrags.begin(); iter != mPartialofFrags.end(); iter++) {
        ...
        b2Vec2 pos = (*iter)->GetPosition();
        float32 angle = (*iter)->GetAngle();
