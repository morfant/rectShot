...
#        self.params.minInertiaRatio = 0.8
    ...
    def detectBlob(self, frame):
        ...
        if len(self.keypoints) > 0: #for test
            ...
            #     print "corner[" + i + "] set"
            #     corner[i][0] = self.keypoints[0].pt[0]
            #     corner[i][1] = self.keypoints[0].pt[1]
            ...
            onCamPoint = [self.keypoints[0].pt[0], self.keypoints[0].pt[1], 1]
            transformedPoint = np.dot(H, onCamPoint)

            # print transformedPoint

            print "trans-X / trans-Y: %d / %d\n" % (round(transformedPoint[0]), round(transformedPoint[1]))

#            oscmsg.setAddress("/laserPos")
#            oscmsg.append(self.keypoints[0].pt[0])
#            oscmsg.append(self.keypoints[0].pt[1])

            # oscmsg.append(round(transformedPoint[0]))
            # oscmsg.append(round(transformedPoint[1]))
#            c.send(oscmsg)
#            oscmsg.clearData()
...
class LaserTracker(object):
    ...
    def __init__(self, cam_width, cam_height, hue_min, hue_max,
        ...
        * ``display_thresholds`` -- if True, additional windows will display
        ...
        self.tl = [0, 0] 
        self.tr = [cam_width, 0]
        self.br = [cam_width, cam_height]
        self.bl = [0, cam_height]
...
#print "Thresholds reading from " + self.thsFile + "...\n"
    ...
    def getH(self, dstWidth, dstHeight): #get Homography

        src = np.array([self.tl, self.tr, self.br, self.bl], dtype = "float32")
    ...
    def printCapturedPoint(self):
        print "TOP-LEFT: %d, %d\n" %(self.tl[0], self.tl[1])
        print "TOP-RIGHT: %d, %d\n" %(self.tr[0], self.tr[1])
        print "BOTTOM-RIGHT: %d, %d\n" %(self.br[0], self.br[1])
        print "BOTTOM-LEFT: %d, %d\n" %(self.bl[0], self.bl[1])
...
#print "Program is stopping..\n"
            ...
            print "space!!!\n"
            self.getH(1440, 900) #Match with projection resolution.

        elif c in [chr(49)]:
            print "1 pressed. Get TOP-LEFT point.\n"
            self.tl = [self.mouseX, self.mouseY]
            self.printCapturedPoint()

        elif c in [chr(50)]:
            print "2 pressed. Get TOP-RIGHT point.\n"
            self.tr = [self.mouseX, self.mouseY]
            self.printCapturedPoint()

        elif c in [chr(51)]:
            print "3 pressed. Get BOTTOM-RIGHT point.\n"
            self.br = [self.mouseX, self.mouseY]
            self.printCapturedPoint()

        elif c in [chr(52)]:
            print "4 pressed. Get BOTTOM-LEFT point.\n"
            self.bl = [self.mouseX, self.mouseY]
            self.printCapturedPoint()
...
#        self.create_and_position_window('LaserPointer', 0, 0)
        self.create_and_position_window('RGB', 0, 0)
        if self.display_thresholds:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                cv2.setMouseCallback('RGB', self.getMousePos)
